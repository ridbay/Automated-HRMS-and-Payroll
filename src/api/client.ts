...
import { Employee, JobRequisition, LeaveRequest, Candidate, Interview } from '../types';
...
// --- ATS: Candidates / Interviews / Offers ---
// All endpoints live under /admin/ats and respond `{ data: ... }`
// (ats.controller.ts), unlike the bare-JSON job-requisitions endpoints above.

const invalidateAts = (queryClient: ReturnType<typeof useQueryClient>, candidateId?: string) => {
  queryClient.invalidateQueries({ queryKey: ['candidates'] });
  queryClient.invalidateQueries({ queryKey: ['interviews'] });
  queryClient.invalidateQueries({ queryKey: ['offers'] });
  invalidateJobRequisitions(queryClient); // applicantsByStage depends on candidate status
  if (candidateId) queryClient.invalidateQueries({ queryKey: ['candidate', candidateId] });
};

export const useCandidates = (filters: { requisitionId?: string; status?: string } = {}) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await fetchWithTenant(`${API_URL}/admin/ats/candidates?${query.toString()}`);
...
// Accepts either a plain object (JSON) or a FormData (when attaching a
// resume) — mirrors useUploadEmployeeDocument's multipart convention.
...
export const candidateResumeUrl = (candidateId: string) => `${API_URL}/admin/ats/candidates/${candidateId}/resume`;
...
export const useSubmitScorecard = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const res = await fetchWithTenant(`${API_URL}/admin/ats/interviews/${interviewId}/scorecard`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
...
export const useSendOffer = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const res = await fetchWithTenant(`${API_URL}/admin/ats/offers/${id}/send`, { method: 'POST' });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
...
export const useRescindOffer = () => {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const res = await fetchWithTenant(`${API_URL}/admin/ats/offers/${id}/rescind`, { method: 'POST' });
...
// --- AI Assistant ---
// Requires a real logged-in session (JWT) — the x-company-id mock-header
// fallback never carries a role, and the assistant's tool access is
// role-scoped server-side (see api/src/services/ai.service.ts).
...
// --- Public careers page (unauthenticated — no tenant headers, plain fetch) ---
...
export const usePublicCareers = (companyIdentifier: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const json = await res.json();
      return json.data as { company: { id: string; name: string; logoUrl?: string }; positions: JobRequisition[] };
    },
    enabled: !!companyIdentifier,
  });
};

export const usePublicPosition = (companyIdentifier: string, requisitionId: string | null) => {
  return useQuery({
    queryKey: ['publicPosition', companyIdentifier, requisitionId],
    queryFn: async () => {
      const res = await fetch(`${API_URL}/public/careers/${companyIdentifier}/${requisitionId}`);
      if (!res.ok) throw new Error('Position not found or no longer open');
      const json = await res.json();
      return json.data as { company: { id: string; name: string; logoUrl?: string }; position: JobRequisition };
    },
...
export const useSubmitPublicApplication = () => {
  return useMutation({
    mutationFn: async ({ companyIdentifier, requisitionId, formData }: { companyIdentifier: string; requisitionId: string; formData: FormData }) => {
      const res = await fetch(`${API_URL}/public/careers/${companyIdentifier}/${requisitionId}/apply`, {
        method: 'POST',
        body: formData,
      });
      if (!res.ok) {
...
// Statutory remittance schedules — one CSV per scheme (PAYE/Pension/NHF/
// NSITF/ITF), same download convention as downloadPayrollBankFile above.
export const downloadRemittanceSchedule = async (runId: string, type: 'paye' | 'pension' | 'nhf' | 'nsitf' | 'itf') => {
  const res = await fetchWithTenant(`${API_URL}/admin/payroll/runs/${runId}/remittance/${type}`);
...
// Slack is the one integration with a real connection flow (an Incoming
// Webhook URL, no OAuth app needed) — kept separate from the generic
// toggle above, which the backend now rejects for key: 'slack'.
...
export const useDisconnectSlack = () => {
  ...
  return useMutation({
    mutationFn: async () => {
      const res = await fetchWithTenant(`${API_URL}/admin/integrations/slack/disconnect`, { method: 'POST' });
...
export const useTestSlack = () => {
  return useMutation({
    mutationFn: async () => {
      const res = await fetchWithTenant(`${API_URL}/admin/integrations/slack/test`, { method: 'POST' });
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
