...
describe('CompanyDocumentService', () => {
  ...
  beforeEach(() => {
    mockDb = {
      ...
      query: {
        companyDocuments: { findFirst: vi.fn(), findMany: vi.fn().mockResolvedValue([]) },
      },
    };
  ...
  describe('create', () => {
    it('rejects a document with no title or content', async () => {
      await expect(service.create('comp-1', actor, { title: '', content: '' })).rejects.toThrow(
    ...
    it('rejects whitespace-only content', async () => {
      await expect(service.create('comp-1', actor, { title: 'Policy', content: '   ' })).rejects.toThrow(
        'Title and content are required.'
      );
    });

    it('creates a text-only document, tagging the uploader', async () => {
      mockDb.query.companyDocuments.findFirst.mockResolvedValueOnce({ id: 'DOC-1', title: 'Remote Work Policy' });

      await service.create('comp-1', actor, { title: 'Remote Work Policy', content: 'Up to 3 days remote per week.' });
    ...
    it('uploads an attached file to R2 and records its key/name', async () => {
      mockDb.query.companyDocuments.findFirst.mockResolvedValueOnce({ id: 'DOC-1' });
      const bucket = { put: vi.fn().mockResolvedValue(undefined) };
      const file = new File(['policy text'], 'handbook.pdf', { type: 'application/pdf' });

      await service.create('comp-1', actor, { title: 'Handbook', content: 'See attached.', file }, bucket as any);
  ...
  describe('delete', () => {
    it('throws when the document does not exist for this company', async () => {
      ...
      await expect(service.delete('comp-1', 'DOC-404')).rejects.toThrow('Document not found');
      expect(mockDb.delete).not.toHaveBeenCalled();
    });

    it('deletes the R2 file (when present) and the row', async () => {
      mockDb.query.companyDocuments.findFirst.mockResolvedValueOnce({ id: 'DOC-1', fileKey: 'companies/comp-1/documents/DOC-1-handbook.pdf' });
      const bucket = { delete: vi.fn().mockResolvedValue(undefined) };

      await service.delete('comp-1', 'DOC-1', bucket as any);

      expect(bucket.delete).toHaveBeenCalledWith('companies/comp-1/documents/DOC-1-handbook.pdf');
      expect(mockDb.delete).toHaveBeenCalled();
    });

    it('deletes the row even when there is no attached file', async () => {
      mockDb.query.companyDocuments.findFirst.mockResolvedValueOnce({ id: 'DOC-1', fileKey: null });
  ...
  describe('search', () => {
    const docs = [
      { id: 'DOC-1', title: 'Remote Work Policy', content: 'Employees may work remotely up to three days a week with manager approval.' },
      { id: 'DOC-2', title: 'Expense Reimbursement', content: 'Submit receipts within 30 days. Remote employees may claim internet costs.' },
      { id: 'DOC-3', title: 'Dress Code', content: 'Business casual is expected in the office.' },
    ];
    ...
    it('returns an excerpt centered on the match, not just the start of the document', async () => {
      mockDb.query.companyDocuments.findMany.mockResolvedValueOnce([
        { id: 'DOC-1', title: 'Long Policy', content: 'A'.repeat(500) + ' parking allowance is two hundred naira ' + 'B'.repeat(500) },
      ]);
    ...
    it('respects the limit and returns only the top N matches', async () => {
      const manyDocs = Array.from({ length: 10 }, (_, i) => ({ id: `DOC-${i}`, title: `Policy ${i} remote`, content: 'remote work' }));
